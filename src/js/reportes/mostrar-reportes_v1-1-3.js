...
import { formatDateText, getSearch, limpiarHTML } from "../helpers/index_v1.js"
import Generador from "../classes/Generador_v1.js";
...
(() => {
  ...
  const renderizarReportes = () => {
    ...
    if(reportes.length === 0) {
      ...
      pEmpty.className = 'py-5 text-center uppercase font-bold text-gray-800 dark:text-gray-300'
      pEmpty.textContent = `No hay ${estados[Number(status) - 1]}`;
      listadoReportes.appendChild(pEmpty);
      document.querySelector('#paginacion').classList.add('hidden')
      return;
    }
  ...
  const aumentarPaso = () => {
    if (actual === Math.floor(totalReportes/limit))  {
      ...
      if(actual > 1) btnAnterior.classList?.remove('hidden');
      return;
    };

    if(reportes.length < limit) {
      btnSiguiente.classList.add('hidden');
      return
    }
