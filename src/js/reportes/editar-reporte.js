(() => {
  ...
  async function enviarReporte(e) {
    ...
    datos.append("folio", obtenerFolio());
    ...
    try {
      const URL = `${location.origin}/api/reporte/editar`;
      const response = await fetch(URL, {
        method: "POST",
        body: datos,
      });
  ...
  function coincidenciaNombre() {
    ...
    usuarios.forEach((usuario) => {
      ...
      if (valor.includes(coincidencia)) {
        ...
        ul.classList.add("space-y-1", "z-40");
  ...
  function mostrarNuevo(res, obj) {
    ...
    btnAcceder.innerHTML = `
        ...
        <p class="font-bold text-xs">Volver</p>
      `;
