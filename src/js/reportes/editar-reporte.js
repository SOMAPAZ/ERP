...
import { formatDateText, limpiarHTML, getSearch } from "../helpers/index.js";
...
(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    selectCategoria.addEventListener("change", (e) => generarIncidencias(e.target.value));
  ...
  const obtenerDatos = async () => {
    [usuarios, prioridades, categorias, reporte] = await Promise.all([
      ...
      GetDatos.consultar(`${location.origin}/api/reporteID?folio=${getSearch().folio}`),
    ])

    agregarOpciones(selectPrioridad, prioridades, reporte.id_priority.id);
    agregarOpciones(selectCategoria, categorias, reporte.id_category.id);
    generarIncidencias(reporte.id_category.id);
  }

  const enviarReporte = async (e) => {
    e.preventDefault();

    const res = await PostDatos.guardarDatos(`${location.origin}/api/reporte/editar`, formulario.querySelectorAll('.input-report'))
  ...
  const generarIncidencias = async(valor) => {
    ...
    const res = await PostDatos.enviarArray(`${location.origin}/api/incidencias`, valor);
    ...
    (res.datos).forEach( incidencia => {
      ...
      reporte.id_incidence.id == incidencia.id ? option.setAttribute('selected', true) : ''
  ...
  const agregarOpciones = (selector, datos, idx) => {
    datos.forEach(dato => {
      const option = document.createElement('OPTION');
      option.value = dato.id;
      option.textContent = dato.name;
      idx == dato.id ? option.setAttribute('selected', true) : ''
